...
class Excgen_ChaoAreaReport {
  static void exportExcel_ChaoAreaReport(
      context,
      NameFile_,
...
////////////-------------------------------------------------------->
    ...
    range.setText('รายงานข้อมูลพื้นที่เช่า (โซน : $Value_Chang_Zone_Area) ');
...
// Protecting the Worksheet by using a Password
    ...
    sheet.getRangeByName('B4').setText('โซนพื้นที่');
    sheet.getRangeByName('C4').setText('รหัสพื้นที่');
    sheet.getRangeByName('D4').setText('ขนาดพื้นที่(ต.ร.ม.)');
    sheet.getRangeByName('E4').setText('ค่าเช่าต่องวด');

    sheet.getRangeByName('F4').setText('เลขที่สัญญา');
    sheet.getRangeByName('G4').setText('เลขที่ใบเสนอราคา');
    ...
    for (int index = 0; index < areaModels.length; index++) {
      dynamic numberColor = (0 * areaModels.length + index) % 2 == 0
          ? globalStyle22
          : globalStyle222;
      ...
      sheet.getRangeByName('D${index + 5}').setNumber(
            areaModels[index].area_c == null
                ? areaModels[index].ln_q == null
                    ...
                    : double.parse(areaModels[index].area_q!)
                : double.parse(areaModels[index].area_c!),
          );
      sheet
          .getRangeByName('E${index + 5}')
          .setNumber(areaModels[index].total == null
              ? areaModels[index].total_q == null
                  ? double.parse(areaModels[index].rent!)
                  : double.parse(areaModels[index].total_q!)
              : double.parse(areaModels[index].total!));

      sheet.getRangeByName('F${index + 5}').setText(
            areaModels[index].cid == null ? '' : '${areaModels[index].cid}',
          );

      sheet.getRangeByName('G${index + 5}').setText(
            areaModels[index].docno == null ? '' : '${areaModels[index].docno}',
          );

      sheet.getRangeByName('H${index + 5}').setText(
            areaModels[index].ldate == null
                ? areaModels[index].ldate_q == null
                    ? ''
                ...
                : DateFormat('dd-MM-yyyy')
                    .format(
                        DateTime.parse('${areaModels[index].ldate} 00:00:00'))
                    .toString(),
          );
    ...
    String path = await FileSaver.instance.saveFile(
        "รายงานข้อมูลพื้นที่เช่า(โซน : $Value_Chang_Zone_Area)", data, "xlsx",
        mimeType: type);
    log(path);
