...
class Excgen_OverdueReport {
  static void exportExcel_overdueReport(
      context,
      NameFile_,
...
// Protecting the Worksheet by using a Password
    ...
    for (int index = 0; index < teNantModels.length; index++) {
      ...
      sheet
          ...
          .setText('${teNantModels[index].cid}');
      ...
      sheet.getRangeByName('D${index + 5}').setText(
            (teNantModels[index].invoice == null)
                ? ''
      ...
      sheet.getRangeByName('K${index + 5}').setNumber(
            (teNantModels[index].total == null)
                ? 0.00
                : double.parse(teNantModels[index].total!),
          );
    }

    sheet.getRangeByName('J${teNantModels.length + 5}').cellStyle =
        globalStyle7;
    sheet.getRangeByName('K${teNantModels.length + 5}').cellStyle =
        globalStyle7;

    sheet.getRangeByName('J${teNantModels.length + 5}').setText(
          'รวม',
        );

    sheet.getRangeByName('K${teNantModels.length + 5}').setFormula(
          '=SUM(K5:K${teNantModels.length + 5 - 1})',
        );
