...
class Excgen_PeopleCho_AllbillReport {
  static void exportExcel_PeopleCho_AllbillReport(
      expModels,
      context,
      NameFile_,
...
// Protecting the Worksheet by using a Password
    ...
    sheet.getRangeByName('I4').columnWidth = 28;
    ...
    sheet.getRangeByName('Q4').columnWidth = 28;
    sheet.getRangeByName('R4').columnWidth = 28;
    sheet.getRangeByName('S4').columnWidth = 28;
    sheet.getRangeByName('T4').columnWidth = 28;
    ...
    sheet.getRangeByName('D4').setText('เบอร์');
    sheet.getRangeByName('E4').setText('ชื่อร้านค้า');
    sheet.getRangeByName('F4').setText('โซนพื้นที่');
    sheet.getRangeByName('G4').setText('รหัสพื้นที่');
    sheet.getRangeByName('H4').setText('ค่าเช่า');

    sheet.getRangeByName('I4').setText('ม.ค.');
    sheet.getRangeByName('J4').setText('ก.พ.');
    sheet.getRangeByName('K4').setText('มี.ค.');
    sheet.getRangeByName('L4').setText('เม.ย.');
    sheet.getRangeByName('M4').setText('พ.ค.');
    sheet.getRangeByName('N4').setText('มิ.ย.');
    sheet.getRangeByName('O4').setText('ก.ค.');
    sheet.getRangeByName('P4').setText('ส.ค.');
    sheet.getRangeByName('Q4').setText('ก.ย.');
    sheet.getRangeByName('R4').setText('ต.ค.');
    sheet.getRangeByName('S4').setText('พ.ย.');
    sheet.getRangeByName('T4').setText('ธ.ค.');
    ...
    for (int i = 0; i < teNantModels.length; i++) {
      ...
      sheet.getRangeByName('Q${i + 5}').cellStyle = numberColor;
      sheet.getRangeByName('R${i + 5}').cellStyle = numberColor;
      sheet.getRangeByName('S${i + 5}').cellStyle = numberColor;
      sheet.getRangeByName('T${i + 5}').cellStyle = numberColor;
      ...
      sheet.getRangeByName('H${i + 5}').setNumber(
            (teNantModels[index].total_contractx == null)
                ? 0.00
                : double.parse('${teNantModels[index].total_contractx}'),
          );
      sheet.getRangeByName('I${i + 5}').setText(transteNantModels_Select
              .where((model) =>
                  model.cid == '${teNantModels[index].cid}' &&
                  model.l_date_m1 != null)
              .map((model) => model.l_date_m1)
              .isEmpty
          ? ''
          : '${transteNantModels_Select.where((model) => model.cid == '${teNantModels[index].cid}').map((model) => '${DateFormat('d MMM', 'th_TH').format(DateTime.parse(model.l_date_m1.toString()))} (${model.l_docno_m1})').join(',')}');
      sheet.getRangeByName('J${i + 5}').setText(
            transteNantModels_Select
                    .where((model) =>
                        model.cid == '${teNantModels[index].cid}' &&
                        model.l_date_m2 != null)
                    .map((model) => model.l_date_m2)
                    .isEmpty
                ? ''
                : '${transteNantModels_Select.where((model) => model.cid == '${teNantModels[index].cid}').map((model) => '${DateFormat('d MMM', 'th_TH').format(DateTime.parse(model.l_date_m2.toString()))} (${model.l_docno_m2})').join(',')}',
          );
      sheet.getRangeByName('K${i + 5}').setText(
            transteNantModels_Select
                    .where((model) =>
                        model.cid == '${teNantModels[index].cid}' &&
                        model.l_date_m3 != null)
                    .map((model) => model.l_date_m3)
                    .isEmpty
                ? ''
                : '${transteNantModels_Select.where((model) => model.cid == '${teNantModels[index].cid}').map((model) => '${DateFormat('d MMM', 'th_TH').format(DateTime.parse(model.l_date_m3.toString()))} (${model.l_docno_m3})').join(',')}',
          );
      ...
      sheet.getRangeByName('L${i + 5}').setText(
            transteNantModels_Select
                    .where((model) =>
                        model.cid == '${teNantModels[index].cid}' &&
                        model.l_date_m4 != null)
                    .map((model) => model.l_date_m4)
                    .isEmpty
                ? ''
                : '${transteNantModels_Select.where((model) => model.cid == '${teNantModels[index].cid}').map((model) => '${DateFormat('d MMM', 'th_TH').format(DateTime.parse(model.l_date_m4.toString()))} (${model.l_docno_m4})').join(',')}',
          );
      sheet.getRangeByName('M${i + 5}').setText(
            transteNantModels_Select
                    .where((model) =>
                        model.cid == '${teNantModels[index].cid}' &&
                        model.l_date_m5 != null)
                    .map((model) => model.l_date_m5)
                    .isEmpty
                ? ''
                : '${transteNantModels_Select.where((model) => model.cid == '${teNantModels[index].cid}').map((model) => '${DateFormat('d MMM', 'th_TH').format(DateTime.parse(model.l_date_m5.toString()))} (${model.l_docno_m5})').join(',')}',
          );
      sheet.getRangeByName('N${i + 5}').setText(
            transteNantModels_Select
                    .where((model) =>
                        model.cid == '${teNantModels[index].cid}' &&
                        model.l_date_m6 != null)
                    .map((model) => model.l_date_m6)
                    .isEmpty
                ? ''
                : '${transteNantModels_Select.where((model) => model.cid == '${teNantModels[index].cid}').map((model) => '${DateFormat('d MMM', 'th_TH').format(DateTime.parse(model.l_date_m6.toString()))} (${model.l_docno_m6})').join(',')}',
          );
      ...
      sheet.getRangeByName('O${i + 5}').setText(
            transteNantModels_Select
                    .where((model) =>
                        model.cid == '${teNantModels[index].cid}' &&
                        model.l_date_m7 != null)
                    .map((model) => model.l_date_m7)
                    .isEmpty
                ? ''
                : '${transteNantModels_Select.where((model) => model.cid == '${teNantModels[index].cid}').map((model) => '${DateFormat('d MMM', 'th_TH').format(DateTime.parse(model.l_date_m7.toString()))} (${model.l_docno_m7})').join(',')}',
          );
      sheet.getRangeByName('P${i + 5}').setText(
            transteNantModels_Select
                    .where((model) =>
                        model.cid == '${teNantModels[index].cid}' &&
                        model.l_date_m8 != null)
                    .map((model) => model.l_date_m8)
                    .isEmpty
                ? ''
                : '${transteNantModels_Select.where((model) => model.cid == '${teNantModels[index].cid}').map((model) => '${DateFormat('d MMM', 'th_TH').format(DateTime.parse(model.l_date_m8.toString()))} (${model.l_docno_m8})').join(',')}',
          );
      sheet.getRangeByName('Q${i + 5}').setText(
            transteNantModels_Select
                    .where((model) =>
                        model.cid == '${teNantModels[index].cid}' &&
                        model.l_date_m9 != null)
                    .map((model) => model.l_date_m9)
                    .isEmpty
                ? ''
                : '${transteNantModels_Select.where((model) => model.cid == '${teNantModels[index].cid}').map((model) => '${DateFormat('d MMM', 'th_TH').format(DateTime.parse(model.l_date_m9.toString()))} (${model.l_docno_m9})').join(',')}',
          );
      sheet.getRangeByName('R${i + 5}').setText(
            transteNantModels_Select
                    .where((model) =>
                        model.cid == '${teNantModels[index].cid}' &&
                        model.l_date_m10 != null)
                    .map((model) => model.l_date_m10)
                    .isEmpty
                ? ''
                : '${transteNantModels_Select.where((model) => model.cid == '${teNantModels[index].cid}').map((model) => '${DateFormat('d MMM', 'th_TH').format(DateTime.parse(model.l_date_m10.toString()))} (${model.l_docno_m10})').join(',')}',
          );
      sheet.getRangeByName('S${i + 5}').setText(
            transteNantModels_Select
                    .where((model) =>
                        model.cid == '${teNantModels[index].cid}' &&
                        model.l_date_m11 != null)
                    .map((model) => model.l_date_m11)
                    .isEmpty
                ? ''
                : '${transteNantModels_Select.where((model) => model.cid == '${teNantModels[index].cid}').map((model) => '${DateFormat('d MMM', 'th_TH').format(DateTime.parse(model.l_date_m11.toString()))} (${model.l_docno_m11})').join(',')}',
          );
      sheet.getRangeByName('T${i + 5}').setText(
            transteNantModels_Select
                    .where((model) =>
                        model.cid == '${teNantModels[index].cid}' &&
                        model.l_date_m12 != null)
                    .map((model) => model.l_date_m12)
                    .isEmpty
                ? ''
                : '${transteNantModels_Select.where((model) => model.cid == '${teNantModels[index].cid}').map((model) => '${DateFormat('d MMM', 'th_TH').format(DateTime.parse(model.l_date_m12.toString()))} (${model.l_docno_m12})').join(',')}',
          );
    }
